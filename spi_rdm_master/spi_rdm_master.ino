...
void irq_pin_handler() { timo.irq_pending = 1; }
...
void setup() {
    ...
    attachInterrupt(digitalPinToInterrupt(timo.irq_pin), irq_pin_handler,
                    FALLING);
    ...
    irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_VERSION_REG),
                              rx_buffer, tx_buffer, 9);
    print_response(irq_flags, rx_buffer, 8);

    /* Making sure module is in TX mode */
    Serial.println("Config:");
    irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_CONFIG_REG), rx_buffer,
                              tx_buffer, 2);
    ...
    while ((rx_buffer[0] & TIMO_CONFIG_RADIO_TX_RX_MODE) == 0) {
        ...
        timo_transfer(TIMO_WRITE_REG_COMMAND(TIMO_CONFIG_REG), rx_buffer,
                      tx_buffer, 2);
        delay(3000);
        irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_CONFIG_REG),
    ...
    irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_INSTALLED_OPTIONS_REG),
     ...
     */
    Serial.println("BLE:");
    irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_BLE_STATUS_REG),
                              rx_buffer, tx_buffer, 2);
    print_response(irq_flags, rx_buffer, 1);
    while (rx_buffer[0] & TIMO_BLE_ENABLED) {
        Serial.println("BLE is on - turning off");
        tx_buffer[0] = 0x00;
        timo_transfer(TIMO_WRITE_REG_COMMAND(TIMO_BLE_STATUS_REG), rx_buffer,
                      tx_buffer, 2);
        delay(3000);
        irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_BLE_STATUS_REG),
                                  rx_buffer, tx_buffer, 2);
    ...
    irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_RF_PROTOCOL_REG),
                              rx_buffer, tx_buffer, 2);
    print_response(irq_flags, rx_buffer, 1);
    while (rx_buffer[0] != rf_protocol) {
        Serial.println("Configured for wrong protocol - changing to G4S");
        tx_buffer[0] = rf_protocol;
        timo_transfer(TIMO_WRITE_REG_COMMAND(TIMO_RF_PROTOCOL_REG), rx_buffer,
                      tx_buffer, 2);
        delay(3000);
        irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_RF_PROTOCOL_REG),
                                  rx_buffer, tx_buffer, 2);
    ...
    irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_STATUS_REG), rx_buffer,
                              tx_buffer, 2);
    print_response(irq_flags, rx_buffer, 1);
    ...
    irq_flags = timo_transfer(TIMO_WRITE_REG_COMMAND(TIMO_IRQ_MASK_REG),
                              rx_buffer, tx_buffer, 2);
    irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_IRQ_MASK_REG),
                              rx_buffer, tx_buffer, 2);
    print_response(irq_flags, rx_buffer, 1);
    ...
    irq_flags = timo_transfer(TIMO_WRITE_REG_COMMAND(TIMO_EXT_IRQ_MASK_REG),
                              rx_buffer, tx_buffer, 5);
    irq_flags = timo_transfer(TIMO_READ_REG_COMMAND(TIMO_EXT_IRQ_MASK_REG),
                              rx_buffer, tx_buffer, 5);
...
void wait_for_rdm_response(void) {
    ...
    while (1) {
        ...
        if (!digitalRead(timo.irq_pin)) {
            ...
            irq_flags =
                timo_transfer(TIMO_NOP_COMMAND, rx_buffer, tx_buffer, 0);
            ...
            if (irq_flags & TIMO_IRQ_EXTENDED_FLAG) {
                ...
                irq_flags =
                    timo_transfer(TIMO_READ_REG_COMMAND(TIMO_EXT_IRQ_FLAGS_REG),
                                  rx_buffer, tx_buffer, 5);
...
uint16_t radio_discover_sub_tree(uint64_t lower, uint64_t upper) {
    ...
    do {
        ...
        if (dub_resp == DiscCollission) {
            ...
            return n_found + radio_discover_sub_tree(lower, mid) +
                   radio_discover_sub_tree(mid + 1, upper);
        }
        ...
        if (dub_resp == DiscUid) {
            if (mute_radio(uid)) {
             ...
             DiscUid); /* keep trying while we're getting single responses */
 ...
 * response
 */
DiscoveryResponseType rdm_dub(uint64_t rx, uint64_t lower, uint64_t upper,
                              uint64_t *uid) {
...
void unmute_all() {
    ...
    timo_transfer(TIMO_WRITE_RDM_COMMAND, rx_buffer, tx_buffer,
                  1 + 6 + req.messageLength + 2);

    wait_for_rdm_response();

    timo_transfer_rdm_response(TIMO_READ_RDM_COMMAND, rx_buffer, tx_buffer,
                               260);
}
...
uint16_t rdm_discover_sub_tree(uint64_t rx, uint64_t lower, uint64_t upper) {
    ...
    do {
        ...
        if (dub_resp == DiscCollission) {
             ...
             * sum of: the number of devices we already found + the number of
             * devices in each half of the tree */
            return n_found + rdm_discover_sub_tree(rx, lower, mid) +
                   rdm_discover_sub_tree(rx, mid + 1, upper);
        }
...
void disc_all(bool incremental) {
    ...
    if (rf_protocol == TIMO_RF_PROTO_G4S) {
        ...
        for (int i = 0; i < n_radios; i++) {
            ...
            Serial.println("...");
            new_found +=
                rdm_discover_sub_tree(radios[i], 0, 0x0000FFFFFFFFFFFF);
            Serial.print("Found ");
            Serial.print(new_found);
            Serial.println(" new...");
            n_found += new_found;
...
void loop() {
    ...
    for (i = 1; i < (n_devices + 1); i++) {
        Serial.print(i);
        Serial.print(") ");
        print_uid(rdm_devices[i - 1]);
        Serial.print(" ");
        print_manufacturer_label(0x00FFFFFFFFFFFF, rdm_devices[i - 1]);
        Serial.print(" ");
    ...
    if (selection == (n_devices + 1)) {
        ...
        /* a device was selected from the list - we will identify it for 5
         * seconds */
        Serial.print("Identifying ");
        print_uid(rdm_devices[selection - 1]);
        Serial.println("...");
 ...
 */
int16_t timo_transfer(uint8_t command, uint8_t *dst, uint8_t *src,
                      uint32_t len) {
    uint8_t irq_flags;
 ...
 * buffer.
 */
int16_t timo_transfer_rdm_response(uint8_t command, uint8_t *dst, uint8_t *src,
                                   uint32_t max_len) {
    uint8_t irq_flags;
